...
def train_model(model,
    ...
    #Above code is training all the layers
    
    
    
    # print(del model.module.backbone)
    
    # training only transformer layers
    parameters = []
    for name, p in model.named_parameters():
        if "bbox" in name:
            parameters.append(p)    
            
    optimizer = torch.optim.AdamW(parameters, lr=cfg.optimizer['lr'], betas=(0.9, 0.999), eps=1e-08,
                                  weight_decay=cfg.optimizer['weight_decay'], amsgrad=False)
